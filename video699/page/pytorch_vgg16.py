...
CONFIGURATION = get_configuration()['PytorchVGG16PageDetector']
...
class VGG16Dataset(torch.utils.data.Dataset):
    def __init__(self, images):
        super(VGG16Dataset, self).__init__()
    ...
    def __getitem__(self, index):
        ...
        img = cv.cvtColor(img.render(VGG16_INPUT_SIZE, VGG16_INPUT_SIZE), cv.COLOR_BGRA2RGB)
...
def _last_hidden_vgg16_layer(images):
    ...
    for image_batch in loader:
        ...
        feature_extractor = torch.nn.Sequential(*list(VGG16_MODEL.children())[:-2])
        activation_batch = feature_extractor(image_batch)  # output now has the features corresponding to input x
        for activations in activation_batch:
            yield activations.squeeze().flatten()


class PyTorchVGG16PageDetector(PageDetectorABC):
